...
void NnetTrainer::Train(const NnetExample &eg) {
  ...
  std::cout << "Number of minibatches processed = ";
  std::cout << num_minibatches_processed_;
  std::cout << "\n\n";
...
void NnetTrainer::TrainInternal(const NnetExample &eg,
  ...
  // store stats.
  //Doubt: only to store the stats?
  NnetComputer computer(config_.compute_config, computation,
                        nnet_, delta_nnet_);
  // give the inputs to the computer object? => No, also update nnet
  ...
  // regularization.
  std::cout << "ApplyRegularization called by TrainInternal\n\n";
...
void NnetTrainer::TrainInternalBackstitch(const NnetExample &eg,
  ...
  if (is_backstitch_step1) {
    ...
    //
    std::cout << "ApplyL2Regularization called by TrainInternalBackstitch.\n\n";
    ApplyL2Regularization(*nnet_,
  ...
  for (; iter != end; ++iter) {
    ...
    if (nnet_->IsOutputNode(node_index)) {
      ...
      ComputeObjectiveFunction(io.features, obj_type, io.name,
        ...
        std::cout << "Initial total objective function is: " << tot_objf << "\n";
	std::cout << "Total weight (number of frames) is: " << tot_weight << "\n";

        // Add the penalty term to the total-objective-loss
	int32 t1l2p = nnet_->GetComponentIndex("t1l2p.affine");
	int32 t1l2s = nnet_->GetComponentIndex("t1l2s.affine");

	Component *comp_a = nnet_->GetComponent(t1l2p);
	Component *comp_b = nnet_->GetComponent(t1l2s);
	AffineComponent *u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	AffineComponent *u_comp_b = dynamic_cast<AffineComponent*>(comp_b);

	CuMatrixBase<BaseFloat> *params_a = NULL;
	CuMatrixBase<BaseFloat> *params_b = NULL;
	KALDI_ASSERT(u_comp_a != NULL && u_comp_b != NULL);

	params_a = &(u_comp_a->LinearParams());
	params_b = &(u_comp_b->LinearParams());

	int32 rows = params_a->NumRows(), cols = params_b->NumCols();
	CuMatrix<BaseFloat> MatAfterMult(rows, cols);
	CuMatrixBase<BaseFloat> *M = NULL;

	MatAfterMult.AddMatMat(1.0 , *params_a, kTrans, *params_b, kNoTrans, 0.0);
	M = &(MatAfterMult);
	BaseFloat penalty = TraceMatMat(*M, *M, kTrans);
	tot_objf -= 0.512*sqrt(penalty);
	M->SetZero();

        std::cout << "Total objective function after 1st update is: " << tot_objf << "\n";

////////////////////////////////////////////////////////////////////////////////////////////////////////

	int32 t2l2p = nnet_->GetComponentIndex("t2l2p.affine");
	int32 t2l2s = nnet_->GetComponentIndex("t2l2s.affine");

	comp_a = nnet_->GetComponent(t2l2p);
	comp_b = nnet_->GetComponent(t2l2s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	...
	MatAfterMult.AddMatMat(1.0 , *params_a, kTrans, *params_b, kNoTrans, 0.0);
	penalty = TraceMatMat(*M, *M, kTrans);
	tot_objf -= 0.512*sqrt(penalty);
	M->SetZero();

        std::cout << "Total objective function after 2nd update is: " << tot_objf << "\n";

///////////////////////////////////////////////////////////////////////////////////////////////////////////	

	int32 t1l3p = nnet_->GetComponentIndex("t1l3p.affine");
	int32 t1l3s = nnet_->GetComponentIndex("t1l3s.affine");

	comp_a = nnet_->GetComponent(t1l3p);
	comp_b = nnet_->GetComponent(t1l3s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	...
	rows = params_a->NumRows();
	cols = params_b->NumRows();
	CuMatrix<BaseFloat> MatAfterMult2(rows, cols);
	CuMatrixBase<BaseFloat> *M2 = NULL;
	std::cout << "rows = " << rows << " and cols = " << cols << "\n";


	MatAfterMult2.AddMatMat(1.0 , *params_a, kNoTrans, *params_b, kTrans, 0.0);
	M2 = &(MatAfterMult2);
	penalty = TraceMatMat(*M2, *M2, kTrans);
	tot_objf -= 0.512*sqrt(penalty);
	M2->SetZero();

        std::cout << "Total objective function after 3rd update is: " << tot_objf << "\n";

///////////////////////////////////////////////////////////////////////////////////////////////////////////


	int32 t2l3p = nnet_->GetComponentIndex("t2l3p.affine");
	int32 t2l3s = nnet_->GetComponentIndex("t2l3s.affine");

	comp_a = nnet_->GetComponent(t2l3p);
	comp_b = nnet_->GetComponent(t2l3s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	...
	M2->SetZero();
        ...
        std::cout << "Total objective function after 4th update is: " << tot_objf << "\n";

///////////////////////////////////////////////////////////////////////////////////////////////////////////


	int32 t1l4p = nnet_->GetComponentIndex("t1l4p.affine");
	int32 t1l4s = nnet_->GetComponentIndex("t1l4s.affine");

	comp_a = nnet_->GetComponent(t1l4p);
	comp_b = nnet_->GetComponent(t1l4s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	...
	M2->SetZero();
        ...
        std::cout << "Total objective function after 5th update is: " << tot_objf << "\n";

///////////////////////////////////////////////////////////////////////////////////////////////////////////

	int32 t2l4p = nnet_->GetComponentIndex("t2l4p.affine");
	int32 t2l4s = nnet_->GetComponentIndex("t2l4s.affine");

	comp_a = nnet_->GetComponent(t2l4p);
	comp_b = nnet_->GetComponent(t2l4s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	...
	M2->SetZero();
        ...
        std::cout << "Total objective function after 6th update is: " << tot_objf << "\n";

///////////////////////////////////////////////////////////////////////////////////////////////////////////

	int32 t1l5p = nnet_->GetComponentIndex("t1l5p.affine");
	int32 t1l5s = nnet_->GetComponentIndex("t1l5s.affine");

	comp_a = nnet_->GetComponent(t1l5p);
	comp_b = nnet_->GetComponent(t1l5s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	...
	M2->SetZero();
        ...
        std::cout << "Total objective function after 7th update is: " << tot_objf << "\n";

///////////////////////////////////////////////////////////////////////////////////////////////////////////

	int32 t2l5p = nnet_->GetComponentIndex("t2l5p.affine");
	int32 t2l5s = nnet_->GetComponentIndex("t2l5s.affine");

	comp_a = nnet_->GetComponent(t2l5p);
	comp_b = nnet_->GetComponent(t2l5s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	...
	M2->SetZero();
        ...
        std::cout << "Total objective function after 8th update is: " << tot_objf << "\n";

///////////////////////////////////////////////////////////////////////////////////////////////////////////

	int32 t1l6p = nnet_->GetComponentIndex("t1l6p.affine");
	int32 t1l6s = nnet_->GetComponentIndex("t1l6s.affine");

	comp_a = nnet_->GetComponent(t1l6p);
	comp_b = nnet_->GetComponent(t1l6s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	KALDI_ASSERT(u_comp_a != NULL && u_comp_b != NULL);
	...
	M2->SetZero();
        ...
        std::cout << "Total objective function after 9th update is: " << tot_objf << "\n";

///////////////////////////////////////////////////////////////////////////////////////////////////////////

	int32 t2l6p = nnet_->GetComponentIndex("t2l6p.affine");
	int32 t2l6s = nnet_->GetComponentIndex("t2l6s.affine");
	  
	comp_a = nnet_->GetComponent(t2l6p);
	comp_b = nnet_->GetComponent(t2l6s);
	u_comp_a = dynamic_cast<AffineComponent*>(comp_a);
	u_comp_b = dynamic_cast<AffineComponent*>(comp_b);
	KALDI_ASSERT(u_comp_a != NULL && u_comp_b != NULL);
	...
	M2->SetZero();
        ...
        std::cout << "Total objective function after 10th update is: " << tot_objf << "\n";
...
void ComputeObjectiveFunction(const GeneralMatrix &supervision,
  ...
  switch (objective_type) {
    case kLinear: {
      std::cout << "Objective type is Linear. *Inside ComputeObjectiveFunction*\n\n";
      // objective is x * y.
      switch (supervision.Type()) {
        case kSparseMatrix: {
	  std::cout << "Supervision type is kSparseMatrix. *Inside ComputeObjectiveFunction*\n\n";
	  ...
	  std::cout << "Supervision type is kFullMatrix. *Inside ComputeObjectiveFunction*\n\n";
	  ...
	  std::cout << "Supervision type is kCompressedMatrix. *Inside ComputeObjectiveFunction*\n\n";
    ...
    case kQuadratic: {
      std::cout << "Objective type is kQuadratic. *Inside ComputeObjectiveFunction*\n\n";
    ...
    default:
      std::cout << "Objective function type not handled. *Inside ComputeObjectiveFunction*\n\n";
